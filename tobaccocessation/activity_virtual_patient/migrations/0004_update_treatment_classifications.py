...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        try:
            reasonable = orm.TreatmentClassification.objects.get(
                rank=2,
                description__startswith='Reasonable but not optimal')
            appropriate = orm.TreatmentClassification.objects.get(rank=1)

            options = orm.TreatmentOption.objects.filter(
                classification=reasonable)
            options.update(classification=appropriate)

            reasonings = orm.TreatmentOptionReasoning.objects.filter(
                classification=reasonable)
            reasonings.update(classification=appropriate)

            feedback = orm.TreatmentFeedback.objects.filter(
                classification=reasonable)
            feedback.update(classification=appropriate)

            reasonable.delete()
        except orm.TreatmentClassification.DoesNotExist:
            pass

        try:
            appropriate = orm.TreatmentClassification.objects.get(
                rank=1,
                description__startswith='Best treatment option')
            appropriate.description = "Appropriate Treatment Choice"
            appropriate.save()
        except orm.TreatmentClassification.DoesNotExist:
            pass

        try:
            ineffective = orm.TreatmentClassification.objects.get(
                rank=3,
                description__startswith='Not likely to be effective')
            ineffective.rank = 2
            ineffective.description = "Unlikely To Be Effective"
            ineffective.save()
        except orm.TreatmentClassification.DoesNotExist:
            pass

        try:
            harmful = orm.TreatmentClassification.objects.get(
                rank=4,
                description__startswith='Potential harm to patient')
            harmful.rank = 3
            harmful.description = "Potentially Harmful"
            harmful.save()
        except orm.TreatmentClassification.DoesNotExist:
            pass
