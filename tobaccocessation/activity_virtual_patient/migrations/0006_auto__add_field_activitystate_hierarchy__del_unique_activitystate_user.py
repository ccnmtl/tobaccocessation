...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Kill all activity first
        orm.ActivityState.objects.all().delete()

        # Removing unique constraint on 'ActivityState', fields ['user']
        db.delete_unique(u'activity_virtual_patient_activitystate', ['user_id'])

        # Adding field 'ActivityState.hierarchy'
        db.add_column(u'activity_virtual_patient_activitystate', 'hierarchy',
                      self.gf('django.db.models.fields.related.ForeignKey')(to=orm['pagetree.Hierarchy']),
                      keep_default=False)

        # Adding unique constraint on 'ActivityState', fields ['hierarchy', 'user']
        db.create_unique(u'activity_virtual_patient_activitystate', ['hierarchy_id', 'user_id'])


    def backwards(self, orm):
        # Removing unique constraint on 'ActivityState', fields ['hierarchy', 'user']
        db.delete_unique(u'activity_virtual_patient_activitystate', ['hierarchy_id', 'user_id'])

        # Deleting field 'ActivityState.hierarchy'
        db.delete_column(u'activity_virtual_patient_activitystate', 'hierarchy_id')

        # Adding unique constraint on 'ActivityState', fields ['user']
        db.create_unique(u'activity_virtual_patient_activitystate', ['user_id'])


    models = {
        u'activity_virtual_patient.activitystate': {
            'Meta': {'unique_together': "(('user', 'hierarchy'),)", 'object_name': 'ActivityState'},
